...
전통적인 낚시 게임의 타이밍 입력이나 반복 조작보다, 방치형 게임에 맞게 **물고기의 움직임을 지켜보는 것 자체가 재미가 되는 플레이**를 목표로 했습니다. 플레이어의 조작은 찌를 놓을 위치를 고르고, 필요할 때 회수를 앞당기는 정도로 줄였습니다.
...
HTML에서 남기기로 한 행동 규칙을 Unity로 옮기고, 찌 배치 → 접근 → 입질 → 회수 → 낚시 가방 도착 → 집계와 리스폰까지 하나의 플레이 흐름으로 구현했습니다.
...
실행 화면을 반복해서 확인한 결과, 몸의 까닥거림과 회피 중 선회 떨림의 원인이 달랐습니다. 몸의 까닥거림은 몸을 굽히는 축과 관련된 문제로 분리해 수정했고, 회피 중 선회 떨림은 회피에 따른 위치 변화가 머리 방향 계산에 영향을 주는 흐름을 따로 살펴봤습니다.
...
![선회 preset 비교](Media/fish-turn-comparison.gif)

*현재 HTML에 보존된 비교용 preset 1(`예전`)과 preset 5(`사행 주기 2배`)를 이어 본 장면입니다.*
...
![현재 수면 진입과 물방울](Media/water-presentation-current.gif)

*현재 Unity 구현에서 수면 진입 직후 파문과 렌즈 물방울이 함께 나타나는 장면입니다.*
...
Unity에서 선택한 행동 규칙과 플레이 흐름은 역할별 코드와 어종별 데이터로 나눴습니다.
...
어획은 입질이나 회수가 시작된 순간이 아니라, 물고기가 낚시 가방에 도착할 때 집계합니다. 이 과정에서 도착 횟수와 집계가 맞는지, 잡힌 물고기가 한 마리씩 다시 생성되는지, 상태가 바뀔 때 위치가 크게 튀지 않는지를 별도로 확인했습니다.
