...
| 물고기 에셋·움직임 | 어종별 길이·폭·높이와 꼬리·지느러미·다리 등의 파라미터로 메시를 생성하고, 정점 셰이더로 몸의 굽힘과 부위별 움직임을 표현 |
| 수면·수중 표현과 연출 | Unity에서 수면 합성, 굴절과 그림자, 착수 파문, 렌즈 물방울, 기포와 카메라 전환을 제작 |

물고기는 별도 모델 파일에 의존하지 않고, 어종별 형태 데이터를 바탕으로 메시를 생성합니다. 몸통의 폭·높이 프로파일과 꼬리·지느러미·오징어 다리를 조합하고, 움직임은 셰이더와 실행 화면을 함께 보며 조정했습니다. 구현을 반복하는 과정에서는 AI를 활용했습니다.
...
렌즈 물방울은 크기와 배치만 조정하는 것으로는 자연스럽게 보이지 않았습니다. 한꺼번에 생기고 같은 시점에 사라지면서 기계적으로 보였기 때문입니다.

물방울마다 수명과 맺히는 속도, 흘러내리기 시작하는 시점을 나눠 두고, 위치·크기와 함께 시간 차이도 조절했습니다. 같은 효과를 여러 개 복제한 느낌보다 화면 전체에 흩어져 맺히고 사라지는 흐름이 보이도록 수정했습니다.

### 착수 파문을 주변 물결과 같은 조명으로 표현했다

찌가 들어간 뒤 퍼지는 파문은 초기 구현에서 밝은 고리가 바깥으로 이동하는 모습이었습니다. 원했던 것은 수면이 굴곡지며 퍼지는 물결이었습니다.

파문의 높이와 기울기를 구하고, 주변 잔물결과 같은 조명 계산에 연결하도록 바꿨습니다. 물결의 한쪽은 빛을 받고 반대쪽은 어두워지며, 바깥으로 퍼질수록 파장이 넓어지고 세기가 줄어들게 했습니다.

수면의 굴절과 잔물결은 절대 시간에 속도를 곱하는 대신 누적 위상을 사용합니다. 연출 중 수면 속도가 바뀔 때 위상이 한 번에 건너뛰며 화면이 튀는 문제를 피하고, 같은 수면 흐름을 유지한 채 속도만 전환하기 위해서입니다.
